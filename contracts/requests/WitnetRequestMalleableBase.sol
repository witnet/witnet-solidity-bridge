...
{   
    ...
    function clone()
        virtual public
        returns (WitnetRequestMalleableBase _instance)
    {
        _instance = WitnetRequestMalleableBase(_clone());
    ...
    function cloneDeterministic(bytes32 _salt)
        ...
        returns (WitnetRequestMalleableBase _instance)
    {
        _instance = WitnetRequestMalleableBase(_cloneDeterministic(_salt));
        _instance.initialize(_request().template);
        Ownable(address(_instance)).transferOwnership(msg.sender);
    }

    function initialized() virtual override public view returns (bool) {
        return _request().template.length != 0;
    }
        ...
        virtual
    {
        ...
        _request().hash = sha256(_request().bytecode);
