        ...
        IWitRandomnessAdmin
{
        ...
        returns (bytes32)
    {
        ...
        if (_status == Witnet.QueryResponseStatus.Ready) {
            return (
                __witOracle.getQueryResultCborBytes(Witnet.QueryId.wrap(_queryId))
                    .toWitnetResult()
                    .asBytes32()
            );
        ...
        )
    {
        ...
        if (_status == Witnet.QueryResponseStatus.Ready) {
            Witnet.QueryResponse memory _queryResponse = __witOracle.getQueryResponse(
                Witnet.QueryId.wrap(_queryId)
            );
        ...
        returns (uint256 _evmUsedFunds)
    {
        ...
        if (__storage().lastRandomizeBlock < block.number) {
            ...
            __randomize.queryId = Witnet.QueryId.unwrap(_queryId);
            ...
            _queryId = Witnet.QueryId.wrap(__storage().randomize_[block.number].queryId);
        }
        ...
        emit Randomizing(tx.origin, _msgSender(), Witnet.QueryId.unwrap(_queryId));
    }
