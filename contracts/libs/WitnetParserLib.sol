...
library WitnetParserLib {
  ...
  function resultFromCborBytes(bytes calldata _cborBytes) external pure returns(Witnet.Result memory) {
  ...
  function asBytes(Witnet.Result memory _result) external pure returns(bytes memory) {
    require(_result.success, "Tried to read bytes value from errored Witnet.Result");
  ...
  function asErrorMessage(Witnet.Result memory _result) public pure returns (Witnet.ErrorCodes, string memory) {
    uint64[] memory error = asRawError(_result);
    if (error.length == 0) {
      return (Witnet.ErrorCodes.Unknown, "Unknown error (no error code)");
    }
    Witnet.ErrorCodes errorCode = supportedErrorOrElseUnknown(error[0]);
  ...
  function asRawError(Witnet.Result memory _result) public pure returns(uint64[] memory) {
    require(!_result.success, "Tried to read error code from successful Witnet.Result");
  ...
  function asBool(Witnet.Result memory _result) external pure returns(bool) {
    require(_result.success, "Tried to read `bool` value from errored Witnet.Result");
  ...
  function asFixed16(Witnet.Result memory _result) external pure returns(int32) {
    require(_result.success, "Tried to read `fixed16` value from errored Witnet.Result");
  ...
  function asInt128(Witnet.Result memory _result) external pure returns(int128) {
    require(_result.success, "Tried to read `int128` value from errored Witnet.Result");
  ...
  function asString(Witnet.Result memory _result) external pure returns(string memory) {
    require(_result.success, "Tried to read `string` value from errored Witnet.Result");
  ...
  function asUint64(Witnet.Result memory _result) external pure returns(uint64) {
    require(_result.success, "Tried to read `uint64` value from errored Witnet.Result");
  ...
  function supportedErrorOrElseUnknown(uint64 _discriminant) private pure returns(Witnet.ErrorCodes) {
