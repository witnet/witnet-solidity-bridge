...
interface IWitnetPriceRouter
{
    ...
    function getPriceFeedCaption(IERC165) external view returns (string memory);
    ...
    function supportedCurrencyPairs() external view returns (bytes32[] memory);

    /// Returns `true` if given pair is currently being served by a compliant price feed contract.
    function supportsCurrencyPair(bytes32 _erc2362id) external view returns (bool);
