...
interface IWitnetRequestBoardRequestor {
    ...
    function destroyResult(uint256 _queryId) external returns (bytes memory);
    ...
    function postRequest(IWitnetRadon _script) external payable returns (uint256);
    ...
    function upgradeRequest(uint256 _queryId) external payable;
}
