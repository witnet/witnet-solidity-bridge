...
contract TestWitnetCBOR {
  ...
  function testBoolDecode() external {
    bool decodedFalse = WitnetCBOR.valueFromBytes(hex"f4").decodeBool();
    bool decodedTrue = WitnetCBOR.valueFromBytes(hex"f5").decodeBool();
    Assert.equal(
      decodedFalse,
      false,
  ...
  function helperDecodeBoolRevert() public pure {
    WitnetCBOR.valueFromBytes(hex"f6").decodeBool();
  }
  ...
  function testUint64DecodeValue() external {
    uint64 decoded = WitnetCBOR.valueFromBytes(hex"1b0020000000000000").decodeUint64();
    Assert.equal(
      uint(decoded),
      9007199254740992,
  ...
  function testInt128DecodeValue() external {
    int128 decoded = WitnetCBOR.valueFromBytes(hex"3bfffffffffffffffe").decodeInt128();
    Assert.equal(
      int(decoded),
  ...
  function testInt128DecodeZeroValue() external {
    int128 decoded = WitnetCBOR.valueFromBytes(hex"00").decodeInt128();
  ...
  function testBytes0DecodeValue() external {
    ...
    bytes memory decoded = WitnetCBOR.valueFromBytes(encoded).decodeBytes();
  ...
  function testBytes32DecodeValueFrom31bytes() external {
    ...
    bytes32 decoded = WitnetCBOR.decodeBytes32(WitnetCBOR.valueFromBytes(encoded));
  ...
  function testStringDecodeDiscriminant() external {
    ...
    Assert.equal(uint(decoded.majorType), 3, "CBOR-encoded String value should be decoded into a WitnetCBOR.CBOR with major type 3");
  }

  function testStringDecodeValue() external {
    bytes memory encoded = hex"6449455446";
    string memory decoded = WitnetCBOR.valueFromBytes(encoded).decodeString();
    string memory expected = "IETF";

    Assert.equal(decoded, expected, "CBOR-encoded String value should be decoded into a WitnetCBOR.CBOR containing the correct String value");
  }
  ...
  function testFloatDecodeSmallestSubnormal() external {
    ...
    int32 decoded = WitnetCBOR.valueFromBytes(encoded).decodeFixed16();
    int32 expected = 0;

    Assert.equal(decoded, expected, "CBOR-encoded Float value should be decoded into a WitnetCBOR.CBOR containing the correct Float value");
  }
  ...
  function testUint64ArrayDecode() external {
    ...
    Assert.equal(
      uint(decoded[0]),
      uint(expected[0]),
      "CBOR-encoded Array of Uint64 values should be decoded into a WitnetCBOR.CBOR containing the correct Uint64 values (error at item 0)"
    );
    Assert.equal(
      uint(decoded[1]),
      uint(expected[1]),
      "CBOR-encoded Array of Uint64 values should be decoded into a WitnetCBOR.CBOR containing the correct Uint64 values (error at item 1)"
    );
    Assert.equal(
      uint(decoded[2]),
      uint(expected[2]),
      "CBOR-encoded Array of Uint64 values should be decoded into a WitnetCBOR.CBOR containing the correct Uint64 values (error at item 2)"
    );
    Assert.equal(
      uint(decoded[3]),
      uint(expected[3]),
      "CBOR-encoded Array of Uint64 values should be decoded into a WitnetCBOR.CBOR containing the correct Uint64 values (error at item 3)"
    );
  }

  function testInt128ArrayDecode() external {
    bytes memory encoded = hex"840121033a002fefd7";
    int128[] memory decoded = WitnetCBOR.valueFromBytes(encoded).decodeInt128Array();
    ...
    Assert.equal(
      decoded[0],
      expected[0],
      "CBOR-encoded Array of Int128 values should be decoded into a WitnetCBOR.CBOR containing the correct Int128 values (error at item 0)"
    );
    Assert.equal(
      decoded[1],
      expected[1],
      "CBOR-encoded Array of Int128 values should be decoded into a WitnetCBOR.CBOR containing the correct Int128 values (error at item 1)"
    );
    Assert.equal(
      decoded[2],
      expected[2],
      "CBOR-encoded Array of Int128 values should be decoded into a WitnetCBOR.CBOR containing the correct Int128 values (error at item 2)"
    );
    Assert.equal(
      decoded[3],
      expected[3],
      "CBOR-encoded Array of Int128 values should be decoded into a WitnetCBOR.CBOR containing the correct Int128 values (error at item 3)"
    );
  }

  function testFixed16ArrayDecode() external {
    bytes memory encoded = hex"84f93c80f9c080f94290f9C249";
    int32[] memory decoded = WitnetCBOR.valueFromBytes(encoded).decodeFixed16Array();
