...
contract TestCBOR {
    ...
    function testBytes4DecodeValue() public {
        ...
        Assert.equal(decoded[0], expected[0], "CBOR-encoded Bytes value should be decoded into a CBOR.Value containing the correct Bytes value (error at item 0)");
        Assert.equal(decoded[1], expected[1], "CBOR-encoded Bytes value should be decoded into a CBOR.Value containing the correct Bytes value (error at item 1)");
        Assert.equal(decoded[2], expected[2], "CBOR-encoded Bytes value should be decoded into a CBOR.Value containing the correct Bytes value (error at item 2)");
        Assert.equal(decoded[3], expected[3], "CBOR-encoded Bytes value should be decoded into a CBOR.Value containing the correct Bytes value (error at item 3)");
    }
    ...
    function testUint64ArrayDecode() public {
        ...
        Assert.equal(uint(decoded[0]), uint(expected[0]), "CBOR-encoded Array of Uint64 values should be decoded into a CBOR.Value containing the correct Uint64 values (error at item 0)");
        Assert.equal(uint(decoded[1]), uint(expected[1]), "CBOR-encoded Array of Uint64 values should be decoded into a CBOR.Value containing the correct Uint64 values (error at item 1)");
        Assert.equal(uint(decoded[2]), uint(expected[2]), "CBOR-encoded Array of Uint64 values should be decoded into a CBOR.Value containing the correct Uint64 values (error at item 2)");
        Assert.equal(uint(decoded[3]), uint(expected[3]), "CBOR-encoded Array of Uint64 values should be decoded into a CBOR.Value containing the correct Uint64 values (error at item 3)");
    }
    ...
    function testInt128ArrayDecode() public {
        ...
        int128[4] memory expected = [int128(1), int128(-2), int128(3), int128(-3141592)];

        Assert.equal(decoded[0], expected[0], "CBOR-encoded Array of Int128 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 0)");
        Assert.equal(decoded[1], expected[1], "CBOR-encoded Array of Int128 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 1)");
        Assert.equal(decoded[2], expected[2], "CBOR-encoded Array of Int128 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 2)");
        Assert.equal(decoded[3], expected[3], "CBOR-encoded Array of Int128 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 3)");
    }
    ...
    function testFixed16ArrayDecode() public {
        ...
        int128[4] memory expected = [int128(11250), int128(-22500), int128(32812), int128(-31425)];

        Assert.equal(decoded[0], expected[0], "CBOR-encoded Array of Fixed16 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 0)");
        Assert.equal(decoded[1], expected[1], "CBOR-encoded Array of Fixed16 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 1)");
        Assert.equal(decoded[2], expected[2], "CBOR-encoded Array of Fixed16 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 2)");
        Assert.equal(decoded[3], expected[3], "CBOR-encoded Array of Fixed16 values should be decoded into a CBOR.Value containing the correct Int128 values (error at item 3)");
    }
    ...
    function testStringArrayDecode() public {
        ...
        string[4] memory expected = ["Hello", "decentralized", "world", "!"];

        Assert.equal(decoded[0], expected[0], "CBOR-encoded Array of String values should be decoded into a CBOR.Value containing the correct String values (error at item 0)");
        Assert.equal(decoded[1], expected[1], "CBOR-encoded Array of String values should be decoded into a CBOR.Value containing the correct String values (error at item 1)");
        Assert.equal(decoded[2], expected[2], "CBOR-encoded Array of String values should be decoded into a CBOR.Value containing the correct String values (error at item 2)");
        Assert.equal(decoded[3], expected[3], "CBOR-encoded Array of String values should be decoded into a CBOR.Value containing the correct String values (error at item 3)");
    }
}
