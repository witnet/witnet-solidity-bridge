        ...
        .then(async value => {
            ...
            if (network) {
                ...
                const contracts = Object.fromEntries(
                    Object.entries(helpers.flattenObject(helpers.getNetworkArtifacts(network)))
...
export function getEvmNetworkByChainId(chainId: number): string | undefined {
    const found = Object.entries(helpers.supportedNetworks()).find(([, config]: [string, any]) => config?.network_id.toString() === chainId.toString())
    if (found) return found[0];
    else return undefined;
}

export function getEvmNetworkSymbol(network: string): string {
    const found = Object.entries(helpers.supportedNetworks()).find(([key,]: [string, any]) => key.toLowerCase() === network.toLowerCase())
