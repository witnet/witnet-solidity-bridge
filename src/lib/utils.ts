	...
	return provider.getNetwork().then(async (value) => {
		...
		if (network) {
			...
			const contracts = Object.fromEntries(
				Object.entries(
					helpers.flattenObject(helpers.getNetworkArtifacts(network)),
				...
				Object.entries(helpers.flattenObject(addresses))
					...
					.map(async ([key, address]) => {
						const bytecode = await provider
							.getCode(address)
							.catch((err) => console.error(`Warning: ${key}: ${err}`))
						if (bytecode?.length && bytecode.length > 2) {
							let impl,
								isUpgradable = false,
								interfaceId,
								version
							const appliance = new Contract(
								address,
								ABIs.WitAppliance,
								provider,
							)
							...
							const upgradable = new Contract(
								address,
								ABIs.WitnetUpgradableBase,
								provider,
							)
							try {
				...
				.then(async (artifacts) => {
					if (Object.fromEntries(artifacts).WitOracle) {
						...
						if (Object.fromEntries(artifacts).WitOraclRadonRegistry) {
							witOracleRadonRegistry = new WitOracleRadonRegistry(
								signer,
								network,
							)
						}
								...
								switch (key) {
									...
									case "WitOracleRadonRequestFactory":
										if (witOracleRadonRegistry) {
											wrapper = await WitOracleRadonRequestFactory.deployed(
												witOracle,
												witOracleRadonRegistry,
											)
										}
										break
									case "WitPriceFeeds":
										wrapper = await WitPriceFeeds.at(
											witOracle,
											artifact.address,
										)
										break
									case "WitPriceFeedsLegacy":
										wrapper = await WitPriceFeedsLegacy.at(
											witOracle,
											artifact.address,
										)
										break
									default:
										if (
											...
											key.startsWith("WitnetRandomness")
										) {
											wrapper = await WitRandomness.at(
												witOracle,
												artifact.address,
											)
										}
...
export function getEvmNetworkByChainId(chainId: number): string | undefined {
	const found = Object.entries(helpers.supportedNetworks()).find(
		([, config]: [string, any]) =>
			config?.network_id.toString() === chainId.toString(),
	)
...
function _intoDataPushReportSolidity(
	report: Witnet.DataPushReport,
): _DataPushReportSolidity {
	return {
...
export function abiEncodeDataPushReportMessage(
	report: Witnet.DataPushReport,
): Witnet.HexString {
...
export function abiEncodeDataPushReportDigest(
	report: Witnet.DataPushReport,
): Witnet.HexString {
...
export function abiEncodePriceFeedUpdateConditions(
...
export function abiEncodeWitOracleQueryParams(
	queryParams: WitOracleQueryParams,
): any {
	return [
