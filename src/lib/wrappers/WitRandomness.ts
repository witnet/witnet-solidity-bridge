...
export class WitRandomness extends WitAppliance {
	...
	static async at(
		...
		try {
			...
			if (oracleAddr !== witOracle.address) {
				throw new Error(
					`WitRandomness at ${target}: mismatching Wit/Oracle address (${oracleAddr})`,
				)
			}
		} catch (error: any) {
			throw new Error(
	...
	public async fetchRandomnessAfter(
		evmBlockNumber: bigint,
	): Promise<Witnet.HexString | undefined> {
	...
	public async fetchRandomnessAfterProof(evmBlockNumber: bigint): Promise<{
		...
		return this.contract.fetchRandomnessAfterProof
	...
	> {
		const logs = await this._legacy.queryFilter(
			"Randomizing",
			options.fromBlock,
			options?.toBlock,
		)
	...
	public async getRandomizeStatus(
		evmBlockNumber: bigint,
	): Promise<RandomizeStatus> {
		return this.contract.getRandomizeStatus
			.staticCall(evmBlockNumber)
			.then((result) => {
	...
	public async randomize(options?: {
		...
		const evmGasPrice =
			options?.evmGasPrice || (await this.provider.getFeeData()).gasPrice || 0n
