...
export class WitPriceFeeds extends WitAppliance {
	...
	static async at(
		...
		try {
			...
			oracleAddr = await priceFeeds.provider
				...
				.then((result) =>
					AbiCoder.defaultAbiCoder().decode(["address"], result),
				)
				.then((result) => result.toString())
		}
		if (oracleAddr !== witOracle.address) {
			throw new Error(
				`${WitPriceFeeds.constructor.name} at ${target}: mismatching Wit/Oracle address (${oracleAddr})`,
			)
		}
		return priceFeeds
	}
	...
	public async createChainlinkAggregator(
		...
		const evmTransaction: ContractTransaction =
			await this.contract.createChainlinkAggregator.populateTransaction(id4)
			...
			.then((tx) => {
				if (!options?.gasPrice && tx?.gasPrice && options?.maxGasPrice) {
					if (tx.gasPrice > options.maxGasPrice) {
						throw new Error(
							`${this.constructor.name}: network gas price too high: ${formatUnits(
								tx.gasPrice,
								9,
	...
	public async settlePriceFeedRadonHash(
		caption: string,
		decimals: number,
		radHash: Witnet.HexString,
		options?: {
			evmConfirmations?: number
			evmGasPrice?: bigint
			evmTimeout?: number
		...
		const evmTransaction: ContractTransaction =
			await this.contract.settlePriceFeedRadonHash.populateTransaction(
				caption,
				decimals,
				radHash,
			)
	...
	public async settlePriceFeedOracle(
		...
		const evmTransaction: ContractTransaction =
			await this.contract.settlePriceFeedOracle.populateTransaction(
				caption,
				decimals,
				typeof oracle === "string"
					? Object.values(PriceFeedOracles).indexOf(oracle)
					: oracle,
				target,
				sources ||
	...
	public async settlePriceFeedMapper(
		...
		const evmTransaction: ContractTransaction =
			await this.contract.settlePriceFeedMapper.populateTransaction(
				caption,
				decimals,
				typeof mapper === "string"
					? Object.values(PriceFeedMappers).indexOf(mapper)
					: mapper,
				dependencies,
			)
		evmTransaction.gasPrice = options?.evmGasPrice || evmTransaction?.gasPrice
	...
	public async removePriceFeed(
		...
		const evmTransaction: ContractTransaction =
			await this.contract.removePriceFeed.populateTransaction(caption, true)
		evmTransaction.gasPrice = options?.evmGasPrice || evmTransaction?.gasPrice
		return this.signer
			.sendTransaction(evmTransaction)
			.then((response) => {
	...
	public async settleDefaultUpdateConditions(
		conditions: PriceFeedUpdateConditions,
		options?: {
			evmConfirmations?: number
			evmGasPrice?: bigint
			evmTimeout?: number
		...
		const evmTransaction: ContractTransaction =
			await this.contract.settleDefaultUpdateConditions.populateTransaction(
	...
	public async settlePriceFeedUpdateConditions(
		caption: string,
		conditions: PriceFeedUpdateConditions,
		options?: {
			evmConfirmations?: number
		...
		const evmTransaction: ContractTransaction =
			await this.contract.settlePriceFeedUpdateConditions.populateTransaction(
				caption,
				abiEncodePriceFeedUpdateConditions(conditions),
			)
		evmTransaction.gasPrice = options?.evmGasPrice || evmTransaction?.gasPrice
	...
	public async clone(
		...
		const tx: ContractTransaction =
			await this.contract.clone.populateTransaction(curator)
	...
	public async getDefaultUpdateConditions(): Promise<PriceFeedUpdateConditions> {
		return this.contract.defaultUpdateConditions.staticCall()
	}

	public async determineChainlinkAggregatorAddress(
		id4: Witnet.HexString,
	): Promise<string> {
		return this.contract.createChainlinkAggregator.staticCall(id4)
	}

	public async getEvmClonableBase(): Promise<string> {
		return this.contract.base.staticCall()
	}

	public async getEvmConsumer(): Promise<Witnet.HexString> {
		return this.contract.consumer.staticCall()
	}
	...
	public async getId4(caption: string): Promise<Witnet.HexString> {
		return this.contract.hash.staticCall(caption).then((id) => id.slice(0, 10))
	}
	...
	public async getPriceNotOlderThan(
		id4: Witnet.HexString,
		age: number,
	): Promise<PriceFeedUpdate> {
		return this.contract
	...
	public async lookupPriceFeed(id4: Witnet.HexString): Promise<PriceFeed> {
		return this.contract.lookupPriceFeed
			.staticCall(id4)
			.then((result: any) => ({
				id: result.id,
				...
				updateConditions: {
					...
					maxDeviationPercentage:
						Number(result.updateConditions.maxDeviation1000) / 10,
					minWitnesses: Number(result.updateConditions.minWitnesses),
				},
				lastUpdate: {
					price:
						Number(result.lastUpdate.price) /
						10 ** Number(-result.lastUpdate.exponent),
					deltaPrice:
						Number(result.lastUpdate.deltaPrice) /
						10 ** Number(-result.lastUpdate.exponent),
	...
	public async lookupPriceFeedCaption(id4: Witnet.HexString): Promise<string> {
		return this.contract.lookupSymbol.staticCall(id4)
	}

	public async lookupPriceFeedExponent(id4: Witnet.HexString): Promise<number> {
		return this.contract.lookupPriceFeedExponent
			.staticCall(id4)
