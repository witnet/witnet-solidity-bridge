...
import type { Addressable, ContractTransactionReceipt, Result } from "ethers";
	...
	public async verifyRadonRequest(
		...
		if (typeof args === "string") {
			...
			if (Array.isArray(args[0])) {
				if (argsCount.length === args.length && !args.find((subargs, index) => subargs.length !== argsCount[index])) {
					encodedArgs = args as Array<string[]>;
				}
			} else if (args.length === argsCount[0] && !args.find((arg) => typeof arg !== "string")) {
				encodedArgs = [args as string[]];
			}
		}
