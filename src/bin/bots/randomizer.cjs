		...
		randomizer
			...
			.then(async (receipt) => {
				...
				return promisePoller({
					...
					taskFn: () =>
						randomizer.isRandomized(randomizeBlock).then(async (isRandomized) => ({
							isRandomized,
					...
					shouldContinue: (err, result) => {
						if (err) {
							...
							if (randomizeWaitBlocks && plus > randomizeWaitBlocks) {
								...
								console.info(`> Awaiting randomness for block ${commas(randomizeBlock)} ... T + ${commas(plus)}`);
							}
						}
					...
					if (result.isRandomized) {
						...
						const trails = await randomizer.fetchRandomnessAfterProof(randomizeBlock);
			...
			.then((result) => {
				if (result.isRandomized) {
					console.info(`> Next randomizing schedule: "${schedule}" at ${scheduleTimezone}`);
				} else {
			...
			.catch((err) => {
				...
				if (isRandomized) {
					...
					console.info(`> Retrying in ${Math.floor(POLLING_MSECS / 1000)} seconds before next randomize ...`);
					// retry immediately a new randomize request
					setTimeout(randomize, POLLING_MSECS);
				}
			});
