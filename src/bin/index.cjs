...
const settings = {
	flags: {
		...
		modals: "List deployed WitOracleRadonRequestModalFactory contracts.",
		randomize: "Pay for a new randomize request.",
		requests: "Includes WitOracleRequest artifacts.",
		templates: "List deployed WitOracleRadonRequestTemplateFactory contracts.",
...
async function main() {
	...
	let options;
	[args, options] = helpers.extractOptionsFromArgs(args, Object.keys(settings.options));
...
function showUsageOptions(options) {
	if (options.length > 0) {
		...
		const maxLength = options
			.map((option) =>
				settings.options[option].param ? settings.options[option].param.length + option.length + 3 : option.length,
			)
			.reduce((prev, curr) => (curr > prev ? curr : prev));
...
function showVersion() {
	console.info(`${lwhite(`Wit/Oracle Solidity CLI v${require("../../package.json").version}`)}`);
}
