	...
	if (logs.length > 0) {
		if (!options["trace-back"]) {
			...
			helpers.traceTable(
				...
				{
					colors: [helpers.colors.white, helpers.colors.mmagenta, helpers.colors.magenta, helpers.colors.gray],
					headlines: [
						"EVM BLOCK:",
					...
					humanizers: [helpers.commas, undefined, undefined, undefined, undefined, undefined],
				},
			);
		}
		console.info(
