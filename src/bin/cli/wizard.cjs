...
module.exports = async (flags = {}) => {
		...
		...(await inquirer.prompt([
			{
				...
				name: "usage",
				message: `What will ${
					isFirstMockup ? "your first" : namedByUser ? `the ${contractName}` : "your next"
	...
	if (appKind === "Randomness" || appKind === "Price" || answers?.parameterized?.split(" ")[0] === "No") {
		// answers = {
		//     ...await inquirer.prompt({
		//         type: "confirm",
		//         name: "includeMocks",
		...
		...(await inquirer.prompt([
			{
				...
				choices: ["Average (+15%)", "Generous (+30%)", "Precautious (+50%)", "Stingy (+5%)"],
			},
		])),
		...answers,
	};
	...
	switch (appKind) {
		...
		case "Public": {
			if (answers?.dynamic.split(" ")[0] === "Yes") {
				...
				templateFile +=
					answers?.callbacks.split(" ")[0] === "Synchronously" ? "_RequestConsumer.tsol" : "_UsingRequest.tsol";
			}
			break;
		}
	}
