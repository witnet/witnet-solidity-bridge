...
module.exports = async (options = {}, args = []) => {
	...
	if (logs?.length > 0) {
		if (options["trace-back"]) {
			helpers.traceTable(
				logs.map((log) => [log.randomizeBlock, log.btr, log.trail?.slice(2), log.randomness]),
				{
					colors: [helpers.colors.white, helpers.colors.lwhite, helpers.colors.magenta, helpers.colors.green],
					headlines: [
						"EVM BLOCK:",
						"B.T.R.",
