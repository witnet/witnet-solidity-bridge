...
module.exports = async (flags = {}, params = []) => {
	...
	if (flags?.templates || flags?.modals) {
		...
		const framework = await helpers.prompter(
			utils.fetchWitOracleFramework(provider).catch((err) => console.error(err)),
		);
...
const includes = (selection, key) => {
	return selection.filter((artifact) => key.toLowerCase().endsWith(artifact.toLowerCase())).length > 0;
};
