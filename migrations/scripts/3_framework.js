    ...
    for (const index in framework[domain]) {
      ...
      if (targetSpecs.isUpgradable) {
        ...
        if (upgradeProxy) {
          ...
          if (targetGithubTag === legacyGithubTag && network !== "develop") {
            ...
            if (legacyClass !== targetClass || legacyVersion !== targetVersion) {
              upgradeProxy = ["y", "yes", ].includes((await
                utils.prompt(`   > Upgrade artifact from ${legacyClass}:${legacyVersion} to \x1b[1;39m${targetClass}:${targetVersion}\x1b[0m? (y/N) `)
              ))
            } else {
              ...
              if (legacyCodeHash !== targetCodeHash) {
                upgradeProxy = ["y", "yes", ].includes((await
                  utils.prompt(`   > Upgrade artifact to \x1b[1;39mlatest compilation of v${targetVersion.slice(0, 6)}\x1b[0m? (y/N) `)
                ))
              }
            }
        ...
        if (implArtifact.address !== targetAddr) {
          ...
          console.info("  ", "                     \x1b[96m -->\x1b[36m", implArtifact.address, "!==", `\x1b[30;43m${targetAddr}\x1b[0m`)
        ...
        //   }
        // }
        if (selection.includes(impl) || utils.isNullAddress(targetBaseAddr) || (await web3.eth.getCode(targetBaseAddr)).length < 3) {
...
async function upgradeCoreBase (proxyAddr, targetSpecs, targetAddr) {
  const initdata = targetSpecs.mutables?.types ? web3.eth.abi.encodeParameters(targetSpecs.mutables.types, targetSpecs.mutables.values) : "0x"
  if (initdata.length > 2) {
  ...
  if (specs.baseDeps.length > 0) {
    ...
    for (const index in specs.baseDeps) {
      ...
      if (utils.isUpgradableArtifact(depsImpl)) {
        ...
        const depsProxySalt = depsVanity ? "0x" + ethUtils.setLengthLeft(ethUtils.toBuffer(depsVanity), 32).toString("hex") : "0x0"
        specs.intrinsics.values.push(await determineProxyAddr(specs.from, depsProxySalt));
      
      } else {
        const depsImplSpecs = await unfoldCoreTargetSpecs(domain, depsImpl, depsBase, specs.from, network, networkArtifacts, networkSpecs, [...ancestors, targetBase])
