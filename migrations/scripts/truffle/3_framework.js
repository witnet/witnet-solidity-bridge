...
const version = `${require("../../../package").version}-${require("node:child_process")
...
module.exports = async (_, network, [, , coreCurator, appsCurator, reporter]) => {
	...
	for (const domain in framework) {
		...
		for (const index in framework[domain]) {
			...
			if (utils.isUpgradableArtifact(impl)) {
				...
				let upgradeProxy =
					targetAddr !== proxyImplAddr && versionCodehashOf(targetVersion) !== versionCodehashOf(legacyVersion);
				...
				const targets = [...utils.getNetworkBaseImplArtifactAddresses(network, domain, addresses, base)];
				for (const ix in targets) {
					...
					if (
						...
						if (target.impl !== impl || fs.existsSync(`migrations/frosts/${domain}/${target.impl}.json`)) {
					...
					console.info("  ", `> contract address:  \x1b[9${color}m`, target.addr, "\x1b[0m");
					if (target.impl === impl) {
...
async function traceDeployedContractInfo(contract, from, targetVersion) {
	...
	try {
		...
		if (targetVersion && versionCodehashOf(deployedVersion) !== versionCodehashOf(targetVersion)) {
			console.info(
				"  ",
				`> contract version:   \x1b[1;39m${deployedVersion.slice(0, 5)}\x1b[0m${deployedVersion.slice(5)} !== \x1b[33m${
					targetVersion
				}\x1b[0m`,
			);
		} else {
...
async function determineProxyAddr(from, nonce) {
	...
	return witnetDeployer.determineProxyAddr.call(salt, { from }).catch((err) => console.error(err));
}
...
function encodeTargetInitCode(target, targetSpecs, networkArtifacts) {
	...
	const targetCode = linkBaseLibs(targetCodeUnlinked, targetSpecs.baseLibs, networkArtifacts);
