...
module.exports = async (truffleDeployer, network, [, master]) => {
	...
	if (utils.isNullAddress(deployerAddr) || (await web3.eth.getCode(deployerAddr)).length < 3) {
		...
		if (fs.existsSync(`migrations/frosts/${impl}.json`)) {
			...
			fs.writeFileSync(`build/contracts/${impl}.json`, fs.readFileSync(`migrations/frosts/${impl}.json`), {
				encoding: "utf8",
				flag: "w",
			});
		} else {
	...
	fs.writeFileSync("build/contracts/WitnetProxy.json", fs.readFileSync("migrations/frosts/WitnetProxy.json"), {
		encoding: "utf8",
		flag: "w",
	});
