...
module.exports = async (truffleDeployer, network, [, master]) => {
	...
	const deployerAddr = utils.getNetworkBaseArtifactAddress(
		network,
		addresses,
		"WitnetDeployer",
	)
	if (
		utils.isNullAddress(deployerAddr) ||
		(await web3.eth.getCode(deployerAddr)).length < 3
	) {
		...
		console.info(
			"  ",
			"> artifact codehash: ",
			web3.utils.soliditySha3(WitnetDeployer.toJSON().deployedBytecode),
		)
		await truffleDeployer.deploy(WitnetDeployer, {
			from:
				settings.getSpecs(network)?.WitnetDeployer?.from ||
				web3.utils.toChecksumAddress(master),
		})
		...
		console.info(
			"  ",
			"> contract address:  \x1b[95m",
			WitnetDeployer.address,
			"\x1b[0m",
		)
	...
	console.info(
		"  ",
		"> artifact codehash: ",
		web3.utils.soliditySha3(WitnetProxy.toJSON().deployedBytecode),
	)
}
