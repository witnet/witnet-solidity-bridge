...
for (const domain in framework) {
    ...
    for (const base in framework[domain]) {
        ...
        if (forceConstructorArgs) {
            ...
            let encodedArgs = web3.eth.abi.encodeParameters(args.types, args.values).slice(2)
            utils.traceVerifyTruffle(network, `${impl} --forceConstructorArgs string:${encodedArgs} --verifiers etherscan,sourcify`)
        } else {
            utils.traceVerifyTruffle(network, `${impl} --verifiers etherscan,sourcify`)
        }
    }
}
